(() => {
  // Selecting all checkbox inputs and other necessary elements
  ...
  // Scroll to the top of the page after 50ms
  setTimeout(() => {
    window.scrollTo(0, 0);
  }, 50);
  ...
  // Generate a random positive or negative number for animation
  ...
  allCheckboxes.forEach((checkbox, index) => {
    checkbox.addEventListener("click", (event) => {
      ...
      if (index === currentIndex) {
        ...
        allCheckboxes[
          currentIndex
        ].style.transform = `translateY(${randomPosOrNeg(
          5 + currentIndex
        )}px)`;
